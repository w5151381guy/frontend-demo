...
這次要做的事情是發個 request 去要這個字串，如果拿到 `Wait` 就等 0.3 秒再發 request 拿，一直重複直到拿到 `Hello World`，如果第 5 次重試還沒拿到就輸出 `can not get`，然後輸出總共試了幾次才拿到 `Hello World`，如果第一次發 request 就拿到就輸出 1，試了 3
